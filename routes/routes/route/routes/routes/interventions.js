...
router.get('/', auth, async (req, res) => {
    try {
        const data = await airtable.getAll(TABLE, {
            sort: [{ field: 'Date intervention', direction: 'desc' }]
        });
        res.json(data);
    } catch (err) {
        console.error('Erreur getInterventions:', err.message);
...
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        console.error('Erreur getInterventionById:', err.message);
        res.status(500).json({ error: 'Erreur serveur' });
    }
});
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        console.error('Erreur createIntervention:', err.message);
        res.status(500).json({ error: 'Erreur serveur' });
    }
});
